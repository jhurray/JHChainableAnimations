...
-(void) setJHAnimationCompletion:(JHAnimationCompletion)animationCompletion {
    objc_setAssociatedObject(self, @selector(JHAnimationCompletion), animationCompletion, OBJC_ASSOCIATION_COPY_NONATOMIC);
}
...
-(JHAnimationCompletion) JHAnimationCompletion {
    return objc_getAssociatedObject(self, @selector(JHAnimationCompletion));
}
...
-(JHChainableColor)makeBorderColor {
    JHChainableColor chainable = JHChainableColor(color) {
        ...
        [self addAnimationCalculationAction:^(UIView *weakSelf) {
            ...
            colorAnimation.fromValue = (__bridge id)(weakSelf.layer.borderColor);
            colorAnimation.toValue = color;
            [weakSelf addAnimationFromCalculationBlock:colorAnimation];
        }];
        [self addAnimationCompletionAction:^(UIView *weakSelf) {
...
-(void) chainLinkDidFinishAnimating {
    ...
    if (self.JHAnimationGroups.count == 0) {
        [self clear];
        if (self.JHAnimationCompletion) {
            JHAnimationCompletion completion = self.JHAnimationCompletion;
            self.JHAnimationCompletion = nil;
            completion();
        }
    }
    else {
...
-(JHChainableAnimationWithCompletion)animateWithCompletion {
    JHChainableAnimationWithCompletion chainable = JHChainableAnimationWithCompletion(duration, completion) {
        ...
        CAAnimationGroup *group = [self.JHAnimationGroups lastObject];
        group.duration = duration;
        self.JHAnimationCompletion = completion;
        [self animateChain];
